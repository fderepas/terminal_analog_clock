    ...
    fn default() -> Self {
        Self {
            entries: vec![
                ...
                Entry {
                    key: "this is an integer".into(),
                    value: Value::Integer { value: 42 },
                },
                Entry {
                    key: "this is a boolean".into(),
    ...
    loop {
        ...
        match ch {
            ...
            32 | 10 | 13 => {
                ...
                if SAVE_WHEN_CHANGE.load(Ordering::SeqCst) {
                    let _ = save_config_to_disk(&cfg,&path);
                }
...
#[allow(dead_code)]
pub fn get_int(path: &str, key: &str) -> Option<i64> {
    let cfg = load_config(path);
    cfg.entries
        .iter()
        .find(|e| e.key == key)
...
/// - For other kinds or missing key: returns `None`.
#[allow(dead_code)]
pub fn get_bool(path: &str, key: &str) -> Option<bool> {
    let cfg = load_config(path);
    cfg.entries
        .iter()
        .find(|e| e.key == key)
...
#[allow(dead_code)]
pub fn set_option(path: &str, key: &str, value: i64) -> Option<i64> {
    if value < 0 {
        return None;
    }
    let mut cfg = load_config(path);
    ...
    if let Some(entry) = cfg.entries.iter_mut().find(|e| e.key == key) {
        ...
        if save_config_to_disk(&cfg, path).is_ok() {
            Some(value)
        } else {
            None
        }
...
#[allow(dead_code)]
pub fn set_int(path: &str, key: &str, value: i64) -> Option<i64> {
    let mut cfg = load_config(path);
...
/// - If key not found, kind mismatch, or save fails: returns `None`.
#[allow(dead_code)]
pub fn set_bool(path: &str, key: &str, value: bool) -> Option<bool> {
    let mut cfg = load_config(path);
...
#[allow(dead_code)]
pub fn set_string(path: &str, key: &str, value: &str) -> Option<bool> {
    let mut cfg = load_config(path);
